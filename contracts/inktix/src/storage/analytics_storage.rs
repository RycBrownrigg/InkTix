...
impl AnalyticsStorage {
    ...
    pub fn create_event_analytics(&mut self, event_id: u32, event: &Event) {
    ...
    pub fn update_platform_stats(&mut self, new_event: bool, new_ticket: bool, revenue: u128) {
        if new_event { self.platform_stats.total_events += 1; }
        if new_ticket { self.platform_stats.total_tickets_sold += 1; }
        self.platform_stats.total_revenue += revenue;
        self.platform_stats.last_updated = 0;
    }
}
