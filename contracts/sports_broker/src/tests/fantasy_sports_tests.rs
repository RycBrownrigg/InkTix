...
pub fn test_fantasy_team_creation() {
    crate::tests::setup_with_test_env(|contract| {
        ...
        assert!(fantasy_team_id > 0, "Fantasy team should have been created with ID > 0");
    });
}
...
pub fn test_fantasy_player_management() {
    crate::tests::setup_with_test_env(|contract| {
        ...
        assert!(result.is_ok(), "Should be able to add second player to team");
...
pub fn test_fantasy_game_week() {
    crate::tests::setup_with_test_env(|contract| {
        ...
        test::set_block_timestamp::<ink::env::DefaultEnvironment>(current_time + 8 * 24 * 60 * 60 * 1000);
